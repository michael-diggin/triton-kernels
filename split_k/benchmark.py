import torch
import triton
from .split_k import splitk_kernel, normal_kernel

DTYPE = torch.float16

providers = ["SplitK", "Kernel","Torch"]

configs = [triton.testing.Benchmark(
            x_names=["V"],  # Argument names to use as an x-axis for the plot
            x_vals=[128 * i for i in range(5, 200, 5)],  # Different possible values for `x_name`
...
def benchmark(V, provider, dtype):
    ...
    if provider == "Kernel":
      ms, min_ms, max_ms = triton.testing.do_bench(lambda: normal_kernel(X, Y), quantiles=quantiles)
