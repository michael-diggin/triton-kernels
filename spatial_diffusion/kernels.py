...
def _splitk_diffusion(
    ...
    # Z = exp(-E.unsqueeze(-1)*D)*(B@Y*M.unsqueeze(-1))
    ...
    # Compute row and column indices for the output Z block
    z_row_indices = pid_m * BLOCK_M + tl.arange(0, BLOCK_M)  # Output rows in Z
    z_col_indices = pid_n * BLOCK_N + tl.arange(0, BLOCK_N)  # Output cols in Z
    ...
    for k in tl.range(0, tl.cdiv(K, BLOCK_K*SPLIT_K)):
      ...
      b_block = tl.load(b_ptrs, mask=(offs_k[None, :] < k_remaining), other=0.0)
      x_block = tl.load(x_ptrs, mask=(offs_k[:, None] < k_remaining), other=0.0)
      m_block = tl.load(m_ptrs, mask=(offs_k[:, None] < k_remaining), other=0.0)

      # Accumulate the result of the dot product
      accumulator += tl.dot(b_block, x_block*m_block)

      b_ptrs += stride_bk*BLOCK_K*SPLIT_K
      x_ptrs += stride_xk*BLOCK_K*SPLIT_K
      m_block += BLOCK_K*BLOCK_K*SPLIT_K

    accumulator.to(z_ptr.dtype.element_ty)
...
_splitk_diffusion.add_pre_run_hook(_zero_output)
